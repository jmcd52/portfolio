<?php
	...
	class Functions {
		...
		public function pageViews($pageName) {
			
			$query = $this->_db->prepare( "SELECT views FROM page_data WHERE page_name=:pagename" );
		...
		public function updateViews($pageName){
			$query = $this->_db->prepare( "UPDATE page_data SET views = views + 1 where page_name=:pagename " );
			$query->bindParam( ":pagename", $pageName, PDO::PARAM_STR );
			$query->execute();
			
		}
		public function resetViews(){
		$query = $this->_db->prepare( "UPDATE page_data SET views=0 WHERE views>0 " );
		$query->execute();
		}
